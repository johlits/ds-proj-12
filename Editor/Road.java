...
public class Road implements XMLSerializable {
	...
	public final static BasicStroke laneSurface = 
		new BasicStroke(laneWidth,BasicStroke.CAP_BUTT,BasicStroke.JOIN_ROUND);
	public final static BasicStroke centerLine = 
		new BasicStroke(3,BasicStroke.CAP_BUTT,BasicStroke.JOIN_BEVEL);
	public final static BasicStroke normal = 
		new BasicStroke(1,BasicStroke.CAP_BUTT,BasicStroke.JOIN_BEVEL);
	public final static BasicStroke laneBackGround = 
		new BasicStroke(laneWidth+2,BasicStroke.CAP_BUTT,BasicStroke.JOIN_ROUND);
	...
	/**
	 * Creates a new <code>Road</code> instance.
	 ...
	 *           <- 8 |       | 7  <-
	 *
	 * @param index The ID number of the Road
	 */
	public Road(int iD) {
		this.iD = iD;
	...
	private boolean checkAngleDiff(double anglediff, double dist) {
		if (dist < 15 && anglediff >= 40) return false;
		if (dist < 25 && anglediff >= 80) return false;
		if (dist < 35 && anglediff >= 90) return false;
		if (anglediff >= 175) return false;
		return true;
	}
